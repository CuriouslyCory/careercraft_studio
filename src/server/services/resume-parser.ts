...
import { processWorkExperience } from "~/server/api/routers/document/work-history";
...
export class ResumeParsingService {
  ...
  async parseResume(
    ...
    const { title = "Parsed Resume", saveDocument = true } = options;
    ...
    try {
      ...
      const counts = await this.storeStructuredData(processedContent, userId);
  ...
  /**
   * Store structured data extracted from resume
   */
