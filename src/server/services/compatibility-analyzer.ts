...
export class CompatibilityAnalyzer {
  ...
  async analyzeCompatibility(
    ...
    // If no UserSkill records found, try to get legacy WorkSkill data
    let normalizedUserSkills = userSkills;
    if (userSkills.length === 0) {
      console.log(
        "No UserSkill records found, checking legacy WorkSkill data...",
      );

      const workHistoryWithSkills = await this.db.workHistory.findMany({
        where: { userId },
        include: {
          skills: true,
        },
      ...
      // Convert legacy WorkSkill data to UserSkill format for compatibility analysis
      const legacySkillsMap = new Map<
        string,
        { count: number; latestJob: string }
      >();
        ...
        for (const workSkill of work.skills) {
          ...
          if (existing) {
            ...
            legacySkillsMap.set(workSkill.name, {
              count: 1,
              latestJob: work.jobTitle,
            });
          }
        }
      }

      // Create temporary UserSkill-like objects for legacy skills
      const legacySkills: UserSkillWithIncludes[] = [];

      for (const [skillName, data] of legacySkillsMap) {
        // Find or create the skill in the normalized table
        let skill = await this.db.skill.findFirst({
          where: {
            OR: [
        ...
        skill ??= await this.db.skill.create({
          data: {
            name: skillName,
        ...
        // Create a UserSkill-like object for analysis
        legacySkills.push({
          id: `legacy_${skill.id}`,
          userId,
          skillId: skill.id,
          ...
          source: "WORK_EXPERIENCE" as SkillSource,
          notes: `Imported from ${data.latestJob}`,
          isVerified: false,
          createdAt: new Date(),
          updatedAt: new Date(),
          workHistoryId: null,
      ...
      normalizedUserSkills = legacySkills;
      console.log(
        `Converted ${legacySkills.length} legacy skills for compatibility analysis`,
      );
    }
